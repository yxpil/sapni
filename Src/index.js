...
async function handleSlashCommand(input) {
  ...
  switch (cmd) {
    ...
    case "version": {
      console.log(`${C.bold + C.cyan}${config.agent.name}${C.reset} v${config.agent.version || "1.1.4-rc7"}`);
      break;
    }
    case "persona":
    case "identity": {
...
async function main() {
  if (process.argv.includes("--version") || process.argv.includes("-v")) {
    loadConfig();
    console.log(config.agent.version || "1.1.4-rc7");
    process.exit(0);
  }
  ...
  const mw = maxWidth();
  console.log(`\n  ${C.bold + C.cyan}${config.agent.name}${C.reset} v${config.agent.version || "1.1.4-rc7"}  ${C.dim}DeepSeek驱动 | 最大宽度: ${mw}列${C.reset}`);
