  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      <Box flexDirection="row" gap={1}>
        ...
        {tool.status !== "done" && <Text color="gray" dimColor>(running...)</Text>}
      </Box>
      
      {tool.args && (
        <Box paddingLeft={2} marginTop={0.5}>
          <Text color="cyan" dimColor>[Args]</Text>
      ...
      {tool.result && (
        <Box paddingLeft={2} marginTop={0.5} borderStyle="round" borderColor="gray" paddingX={2} paddingY={1}>
          <Text color="cyan" dimColor>[Result]</Text>
          <Box marginTop={0.5}>
            {(() => {
              ...
              return (
                <Box flexDirection="column">
                  ...
                  {lines.length > 8 && <Text color="gray" dimColor>... (+{lines.length - 8} lines)</Text>}
                </Box>
              );
            })()}
          </Box>
    ...
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="gray" paddingX={2} paddingY={1}>
        <Box flexDirection="row" justifyContent="space-between">
          ...
          <Text color="gray" dimColor>{completedCount}/{tools.length}</Text>
        </Box>
        <Box flexDirection="column" marginTop={1} gap={0.5}>
          {tools.map((t, i) => (
            <Box key={i} flexDirection="row" gap={1}>
              ...
              {runningCount === 0 && (
                <Text color="gray" dimColor>- {t.result?.length > 50 ? t.result.slice(0, 50) + "..." : t.result}</Text>
              )}
            </Box>
          ))}
        </Box>
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="gray" paddingX={2} paddingY={1}>
      <Box flexDirection="row" justifyContent="space-between" marginBottom={1}>
        <Text color="gray" dimColor>{"─".repeat(10)} Tool Calls ({tools.length})</Text>
        <Text color="gray" dimColor>{completedCount} done{runningCount > 0 ? `, ${runningCount} running` : ""}</Text>
      </Box>
